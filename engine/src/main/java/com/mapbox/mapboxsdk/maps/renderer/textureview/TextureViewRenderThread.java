...
class TextureViewRenderThread extends Thread implements TextureView.SurfaceTextureListener {
  ...
  private static final String TAG = TextureViewRenderThread.class.getSimpleName();
      ...
      while (true) {
        ...
        switch (swapError) {
          ...
          case EGL11.EGL_CONTEXT_LOST:
            ...
            break;
          default:
            Logger.w(TAG, "eglSwapBuffer error: %s. Waiting or new surface", swapError);
  ...
  private static class EGLHolder {
    ...
    boolean makeCurrent() {
      if (!egl.eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext)) {
        ...
        Logger.w(TAG, "eglMakeCurrent: %s", egl.eglGetError());
        return false;
      }
    ...
    private void destroySurface() {
      ...
      if (!egl.eglDestroySurface(eglDisplay, eglSurface)) {
        Logger.w(TAG, "Could not destroy egl surface. Display %s, Surface %s", eglDisplay, eglSurface);
      }
    ...
    private void destroyContext() {
      ...
      if (!egl.eglDestroyContext(eglDisplay, eglContext)) {
        Logger.w(TAG, "Could not destroy egl context. Display %s, Context %s", eglDisplay, eglContext);
      }
    ...
    private void terminate() {
      ...
      if (!egl.eglTerminate(eglDisplay)) {
        Logger.w(TAG, "Could not terminate egl. Display %s", eglDisplay);
      }
